...
__all__ = [
    "IndusGrowQueryConfig",
    "IndusGrow",
]


class IndusGrowQueryConfig(BaseRequestConfig):
    """"""
...
class IndusGrow(BaseRequestData):
    """"""
    def __init__(
            self,
            query_config: Optional[IndusGrowQueryConfig] = None,
            verbose: Optional[bool] = False,
            logger: Optional[Callable] = None,
            **kwargs: Any
    ):
        if query_config is None:
            self.query_config = IndusGrowQueryConfig.model_validate(kwargs)
        else:
