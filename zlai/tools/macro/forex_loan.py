...
__all__ = [
    "ForexLoanQueryConfig",
    "ForexLoan",
]


class ForexLoanQueryConfig(BaseRequestConfig):
    """"""
...
class ForexLoan(BaseRequestData):
    """"""
    def __init__(
            self,
            query_config: Optional[ForexLoanQueryConfig] = None,
            verbose: Optional[bool] = False,
            logger: Optional[Callable] = None,
            **kwargs: Any
    ):
        if query_config is None:
            self.query_config = ForexLoanQueryConfig.model_validate(kwargs)
        else:
